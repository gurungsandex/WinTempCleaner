...
.PARAMETER MaxAgeDays
    Only delete files older than this many days. Default is 1.
...
function Start-UserCleanup {
    ...
    Clear-Folder "$ProfilePath\AppData\Local\Temp"                                          -Label "Temp Files"         -Recurse -AgeDays 0
    Clear-Folder "$ProfilePath\AppData\Local\Microsoft\Windows\INetCache"                   -Label "Browser Cache"      -Recurse -AgeDays 0
    Clear-Folder "$ProfilePath\AppData\Roaming\Microsoft\Windows\Recent"                    -Label "Recent Items"                -AgeDays 0
    Clear-Folder "$ProfilePath\AppData\Local\Microsoft\Windows\Explorer"                    -Label "Thumbnail Cache"             -AgeDays 0
    Clear-Folder "$ProfilePath\AppData\Local\Microsoft\Teams\tmp"                           -Label "Teams Temp"         -Recurse -AgeDays 0
    Clear-Folder "$ProfilePath\AppData\Local\Google\Chrome\User Data\Default\Cache"         -Label "Chrome Cache"       -Recurse -AgeDays 0
    Clear-Folder "$ProfilePath\AppData\Local\Mozilla\Firefox\Profiles"                      -Label "Firefox Cache"      -Recurse -AgeDays 0
    Clear-Folder "$ProfilePath\AppData\Local\CrashDumps"                                    -Label "Crash Dumps"        -Recurse -AgeDays 0
}
...
Write-Log "  Age filter: Files older than $MaxAgeDays day(s)"
Write-Log "  Disk now : $(Get-DiskSpace)"
Write-Log "========================================================"

# Find all real user profiles - wrap in @() to ensure array even if only 1 profile found
